<div class="d-flex align-items-center justify-content-between">
    <a href="{{ url('/users/profile/' . $follower->current_user->id) }}" class="text-decoration-none d-flex align-items-center gap-2">
        @php
            $colors = [
                '#FF5733',
                '#2E9944',
            ...
            $colorIndex = $follower->current_user->id % count($colors);
            $color = $colors[$colorIndex];
        @endphp
        @if ($follower->current_user->image)
            <img src="{{ asset('storage/' . $follower->current_user->image) }}" alt="{{ $follower->current_user->name }}"
        ...
        @else
            ...
            {{ $color }};">
                {{ substr($follower->current_user->name, 0, 1) }}
            </b> 
        @endif
