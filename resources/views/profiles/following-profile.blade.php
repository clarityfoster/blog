<div class="d-flex align-items-center justify-content-between">
    <a href="{{ url('/users/profile/' . $following->user->id) }}" class="text-decoration-none d-flex align-items-center gap-2">
        @php
            $colors = [
                '#FF5733',
                '#2E9944',
            ...
            $colorIndex = $following->user->id % count($colors);
            $color = $colors[$colorIndex];
        @endphp
        @if ($following->user->image)
            <img src="{{ asset('storage/' . $following->user->image) }}" alt="{{ $following->user->name }}"
        ...
        @else
            ...
            {{ $color }};">
                {{ substr($following->user->name, 0, 1) }}
            </b>    
        @endif
        <b class="h5 text-dark">{{ $following->user->name }}</b>
    </a>
